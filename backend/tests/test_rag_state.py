...
async def wire_db(test_db, monkeypatch):
...
async def test_index_resume_marks_row_ok_on_success(test_db):
    await set_active_signature("openai__m")
    with patch("src.memory.rag.active_collection", new_callable=AsyncMock) as ac, \
         patch("src.memory.rag.index_resume_into") as into:
        ac.return_value = object()  # any truthy collection
        into.return_value = None

        await index_resume(7, "A.pdf", "alpha text")
...
async def test_index_resume_marks_failed_when_chroma_raises(test_db):
    await set_active_signature("openai__m")
    with patch("src.memory.rag.active_collection", new_callable=AsyncMock) as ac, \
         patch("src.memory.rag.index_resume_into") as into:
...
async def test_index_resume_noop_when_no_active_signature(test_db):
    # Active signature still NULL -- no indexing happens, row stays clean
    with patch("src.memory.rag.active_collection", new_callable=AsyncMock) as ac:
        ac.return_value = None
        await index_resume(7, "A.pdf", "alpha")
