...
class TestRunPlanning:
    ...
    def test_creates_plan_and_opening_turn(self, mock_get_model, test_db):
        ...
        mock_llm.invoke.return_value = _mock_plan_response()
...
class TestProcessInterviewTurn:
    @patch("src.agents.interview_engine.get_interview_model")
    def test_saves_both_turns_and_returns_response(self, mock_get_model, test_db):
        ...
        mock_llm = MagicMock()
        mock_llm.with_structured_output.return_value = mock_llm
        mock_llm.invoke.side_effect = [_mock_plan_response(), _mock_turn_response()]
        mock_get_model.return_value = mock_llm
        ...
        result = process_interview_turn(session_id, "I would use a hash-based approach.")
    ...
    @patch("src.agents.interview_engine.get_interview_model")
    def test_fallback_on_structured_output_failure(self, mock_get_model, test_db):
        from src.agents.interview_engine import run_planning, process_interview_turn
        ...
        mock_llm.invoke.return_value = MagicMock(content="Could you tell me more about that?")
        mock_get_model.return_value = mock_llm
        ...
        run_planning(session_id)
        result = process_interview_turn(session_id, "Some answer.")
...
class TestRunScoring:
    @patch("src.agents.interview_engine.get_interview_model")
    def test_produces_results_and_completes_session(self, mock_get_model, test_db):
        ...
        run_planning(session_id)
        process_interview_turn(session_id, "My answer.")
