...
class TestCreateChatModel:
    ...
    @patch("src.models.provider.load_credential", new_callable=AsyncMock)
    @patch("src.models.provider.ChatOpenAI")
    async def test_openai_uses_responses_api(self, mock_chat, mock_cred):
        from src.models.provider import _create_chat_model

        mock_cred.return_value = _make_profile()
        await _create_chat_model("openai", "gpt-5.4")
    ...
    @patch("src.models.provider.load_credential", new_callable=AsyncMock)
    @patch("src.models.provider.ChatOpenAI")
    async def test_openai_codex_sends_attribution_headers(self, mock_chat, mock_cred):
        import base64, json
        from src.models.provider import _create_chat_model
        ...
        await _create_chat_model("openai-codex", "gpt-5.4")
    ...
    async def test_anthropic_uses_anthropic_client(self, mock_chat, mock_cred):
        from src.models.provider import _create_chat_model

        mock_cred.return_value = _make_profile(provider="anthropic")
        await _create_chat_model("anthropic", "claude-sonnet-4-6")
        mock_chat.assert_called_once_with(
    ...
    @patch("src.models.provider.load_credential", new_callable=AsyncMock)
    @patch("src.models.provider.ChatAnthropic")
    async def test_kimi_uses_anthropic_client_with_base_url(self, mock_chat, mock_cred):
        from src.models.provider import _create_chat_model

        mock_cred.return_value = _make_profile(provider="kimi")
        await _create_chat_model("kimi", "kimi-k2.5")
        mock_chat.assert_called_once_with(
            model="kimi-k2.5",
            api_key="fake-key",
    ...
    @patch("src.models.provider.load_credential", new_callable=AsyncMock)
    @patch("src.models.provider.ChatOpenAI")
    async def test_openrouter_uses_completions_no_responses_flag(self, mock_chat, mock_cred):
        from src.models.provider import _create_chat_model

        mock_cred.return_value = _make_profile(provider="openrouter")
        await _create_chat_model("openrouter", "deepseek/deepseek-v3")
        mock_chat.assert_called_once_with(
    ...
    @patch("src.models.provider.load_credential", new_callable=AsyncMock)
    async def test_no_credentials_raises(self, mock_cred):
        from src.models.provider import _create_chat_model
    ...
    async def test_unknown_provider_raises(self):
        ...
        with pytest.raises(ValueError, match="Unknown provider"):
            await _create_chat_model("nonexistent", "some-model")
...
class TestGetChatModelFallback:
    ...
    async def test_fallback_on_primary_failure(self, mock_create, mock_config):
        ...
        await get_chat_model("default")
        assert mock_create.call_count == 2
        mock_create.assert_called_with("openai-codex", "gpt-5.4")

    @patch("src.models.provider.load_model_config", new_callable=AsyncMock)
    @patch("src.models.provider._create_chat_model", new_callable=AsyncMock)
    async def test_no_fallback_propagates_error(self, mock_create, mock_config):
        from src.models.provider import get_chat_model
    ...
    @patch("src.models.provider.load_model_config", new_callable=AsyncMock)
    @patch("src.models.provider._create_chat_model", new_callable=AsyncMock)
    async def test_primary_success_no_fallback_attempt(self, mock_create, mock_config):
        from src.models.provider import get_chat_model
...
class TestLiveProviderSmoke:
    ...
    async def test_openai_live(self):
        ...
        key = await load_api_key("openai")
        if not key:
            pytest.skip("No openai credentials configured")
        model = await _create_chat_model("openai", "gpt-4o-mini")
        result = model.invoke("Reply with the word hello")
        assert result.content

    async def test_openai_codex_live(self):
        from src.auth.credentials import load_api_key
        from src.models.provider import _create_chat_model

        key = await load_api_key("openai-codex")
        if not key:
            pytest.skip("No openai-codex credentials configured")
        model = await _create_chat_model("openai-codex", "gpt-5.4")
        result = model.invoke("Reply with the word hello")
    ...
    async def test_anthropic_live(self):
        ...
        key = await load_api_key("anthropic")
        if not key:
            pytest.skip("No anthropic credentials configured")
        model = await _create_chat_model("anthropic", "claude-haiku-4-5")
        result = model.invoke("Reply with the word hello")
        assert result.content

    async def test_kimi_live(self):
        from src.auth.credentials import load_api_key
        from src.models.provider import _create_chat_model

        key = await load_api_key("kimi")
        if not key:
            pytest.skip("No kimi credentials configured")
        model = await _create_chat_model("kimi", "kimi-k2")
        result = model.invoke("Reply with the word hello")
        assert result.content
